...
export function FooterBar({ data }: { data: WeatherData }) {
  return (
    <footer className="grid grid-cols-2 gap-3 border-t border-border px-1 pt-4 md:grid-cols-4">
      {items.map((item) => {
        const Icon = item.icon
        return (
          <div key={item.label} className="flex items-center gap-3 rounded-xl border border-border bg-card/60 px-4 py-3">
            <span className="grid size-9 place-items-center rounded-lg" style={{ color: item.color, backgroundColor: `${item.color}1f` }}>
